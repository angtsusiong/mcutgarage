...
(function() {
    ...
    this.datatables.initialize = function() {
        ...
        // 列表頁的 datatables 設定
        $('.articles_table').each(function(){
            var table = $(this);
            var tab_key = table.data('tab_key');

            var editor = new $.fn.dataTable.Editor({
                ajax: {
                    url: '/articles/update_row',
                    type: 'PATCH'
                },
                table: "#"+ tab_key +"_articles_table",
                idSrc: "DT_RowId",
                fields: [
                    {
                    fieldInfo: '',
                    label: 'title',
                    label: I18n.t('activerecord.attributes.article.title'),
                    name: 'title',
                },
                {
                    fieldInfo: '',
                    label: 'content',
                    label: I18n.t('activerecord.attributes.article.content'),
                    name: 'content',
                },
                {
                    fieldInfo: '',
                    label: 'status',
                    label: I18n.t('activerecord.attributes.article.status'),
                    name: 'status',
                },
                {
                    type: 'datetime',
                ...
                fieldInfo: '',
                    label: 'published_at',
                    label: I18n.t('activerecord.attributes.article.published_at'),
                    name: 'published_at',
                },
                ],
            });
            ...
            var datatable = table.DataTable({
                ...
                },
                {
                    ...
                    visible: true,
                    orderable: false,
                    className: 'editable col-status',
                    editField: 'status',
                },
                {
                    data: 'published_at',
                    name: 'published_at_between',
                    visible: true,
                    orderable: false,
                    className: 'editable col-published_at',
                ...
                },
                    {
                        ...
                        render: function ( data, type, full, meta ) {
                            var id = data.id;
                            var actions = '<a href="/articles/' + id + '?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-eye fa-lg"></span></a>';
                            actions += '<a href="/articles/' + id + '/edit?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-pencil fa-lg"></span></a>';
                            actions += '<a href="/articles/' + id + '" data-confirm="確定刪除?" class="btn btn-default" rel="nofollow" data-method="delete"><span class="fa fa-trash fa-lg"></span></a>';
                            return actions;
                        },
                    },
                ],
            ...
            $("#"+ tab_key +"_articles_table").on( 'click', 'tbody td.editable', function (e) {
        ...
        // 點擊頁籤
        $('.article__tab').on('click', function(){
            var tab = $(this);
...
$(function() {
    articles.datatables.initialize();
});
