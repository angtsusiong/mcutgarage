var admin_activities = admin_activities || {};
$.extend(true, admin_activities, {
  datatables: {}
});
...
(function() {
    ...
    this.datatables.initialize = function() {
        ...
        // 列表頁的 datatables 設定
        $('.activities_table').each(function(){
            var table = $(this);
            var tab_key = table.data('tab_key');

            var editor = new $.fn.dataTable.Editor({
                ajax: {
                    url: '/admin/activities/update_row',
                    type: 'PATCH'
                },
                table: "#"+ tab_key +"_activities_table",
                idSrc: "DT_RowId",
                fields: [
                {
                    fieldInfo: '',
                    label: 'name',
                    label: I18n.t('activerecord.attributes.activity.name'),
                    name: 'name',
                },
                {
                    fieldInfo: '',
                    label: 'default',
                    label: I18n.t('activerecord.attributes.activity.default'),
                    name: 'default',
                },
                {
                    fieldInfo: '',
                    label: 'is_active',
                    label: I18n.t('activerecord.attributes.activity.is_active'),
                    name: 'is_active',
                },
                ],
            });
            ...
            var datatable = table.DataTable({
                ...
                dom: 'Brtip"bottom"l',
                ajax: {
                    url: '/admin/activities/datatables.json',
                    dataSrc: 'data',
                    type: 'POST',
                    data: {
                    },
                ...
                },
                {
                    ...
                    visible: true,
                    orderable: false,
                    className: 'editable col-default',
                    editField: 'default',
                },
                {
                    data: 'is_active',
                ...
                },
                    {
                        ...
                        render: function ( data, type, full, meta ) {
                            var id = data.id;
                            var actions = '<a href="/admin/activities/' + id + '?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-eye fa-lg"></span></a>';
                            actions += '<a href="/admin/activities/' + id + '/edit?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-pencil fa-lg"></span></a>';
                            actions += '<a href="/admin/activities/' + id + '" data-confirm="確定刪除?" class="btn btn-default" rel="nofollow" data-method="delete"><span class="fa fa-trash fa-lg"></span></a>';
                            return actions;
                        },
                    },
                ],
            ...
            $("#"+ tab_key +"_activities_table").on( 'click', 'tbody td.editable', function (e) {
        ...
        // 點擊頁籤
        $('.activity__tab').on('click', function(){
            var tab = $(this);
...
$(function() {
    admin_activities.datatables.initialize();
});
