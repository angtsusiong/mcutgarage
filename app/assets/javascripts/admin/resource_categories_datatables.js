var admin_resource_categories = admin_resource_categories || {};
$.extend(true, admin_resource_categories, { 
	datatables: {}
});
...
(function() {
    ...
    this.datatables.initialize = function() {
        ...
        // 列表頁的 datatables 設定
        $('.resource_categories_table').each(function(){
            var table = $(this);
            var tab_key = table.data('tab_key');

            var editor = new $.fn.dataTable.Editor({
                ajax: {
                    url: '/admin/resource_categories/update_row',
                    type: 'PATCH'
                },
                table: "#"+ tab_key +"_resource_categories_table",
                idSrc: "DT_RowId",
                fields: [
                    {
                    fieldInfo: '',
                    label: 'name',
                    label: I18n.t('activerecord.attributes.resource_category.name'),
                    name: 'name',
                },
                ],
            });
            ...
            var datatable = table.DataTable({
                ...
                dom: 'Brtip"bottom"l',
                ajax: {
                    url: '/admin/resource_categories/datatables.json',
                    dataSrc: 'data',
                    type: 'POST',
                    data: {
                    },
                ...
                columns:[
                ...
                },
                    {
                        ...
                        render: function ( data, type, full, meta ) {
                            var id = data.id;
                            var actions = '<a href="/admin/resource_categories/' + id + '?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-eye fa-lg"></span></a>';
                            actions += '<a href="/admin/resource_categories/' + id + '/edit?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-pencil fa-lg"></span></a>';
                            actions += '<a href="/admin/resource_categories/' + id + '" data-confirm="確定刪除?" class="btn btn-default" rel="nofollow" data-method="delete"><span class="fa fa-trash fa-lg"></span></a>';
                            return actions;
                        },
                    },
                ],
            ...
            $("#"+ tab_key +"_resource_categories_table").on( 'click', 'tbody td.editable', function (e) {
        ...
        // 點擊頁籤
        $('.resource_category__tab').on('click', function(){
            var tab = $(this);
...
$(function() {
    admin_resource_categories.datatables.initialize();
});
