var admin_project_categories = admin_project_categories || {};
$.extend(true, admin_project_categories, {
  datatables: {}
});
...
(function() {
    ...
    this.datatables.initialize = function() {
        ...
        // 列表頁的 datatables 設定
        $('.project_categories_table').each(function(){
            var table = $(this);
            var tab_key = table.data('tab_key');

            var editor = new $.fn.dataTable.Editor({
                ajax: {
                    url: '/admin/project_categories/update_row',
                    type: 'PATCH'
                },
                table: "#"+ tab_key +"_project_categories_table",
                idSrc: "DT_RowId",
                fields: [
                    {
                    fieldInfo: '',
                    label: 'name',
                    label: I18n.t('activerecord.attributes.project_category.name'),
                    name: 'name',
                },
                {
                    fieldInfo: '',
                    label: 'brief_intro',
                    label: I18n.t('activerecord.attributes.project_category.brief_intro'),
                    name: 'brief_intro',
                },
                {
                    fieldInfo: '',
                    label: 'is_active',
                    label: I18n.t('activerecord.attributes.project_category.is_active'),
                    name: 'is_active',
                },
                ],
            });
            ...
            var datatable = table.DataTable({
                ...
                dom: 'Brtip"bottom"l',
                ajax: {
                    url: '/admin/project_categories/datatables.json',
                    dataSrc: 'data',
                    type: 'POST',
                    data: {
                    },
                ...
                {
                    data: 'brief_intro',
                    name: 'brief_intro_cont',
                    visible: true,
                    orderable: false,
                    className: 'editable col-brief_intro',
                    editField: 'brief_intro',
                },
                {
                    data: 'is_active',
                    name: 'is_active_true',
                    visible: true,
                    orderable: false,
                    className: 'editable col-is_active',
                    editField: 'is_active',
                },
                    {
                        // 操作
                        ...
                        render: function ( data, type, full, meta ) {
                            var id = data.id;
                            var actions = '<a href="/admin/project_categories/' + id + '?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-eye fa-lg"></span></a>';
                            actions += '<a href="/admin/project_categories/' + id + '/edit?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-pencil fa-lg"></span></a>';
                            actions += '<a href="/admin/project_categories/' + id + '" data-confirm="確定刪除?" class="btn btn-default" rel="nofollow" data-method="delete"><span class="fa fa-trash fa-lg"></span></a>';
                            return actions;
                        },
                    },
                ],
            ...
            $("#"+ tab_key +"_project_categories_table").on( 'click', 'tbody td.editable', function (e) {
        ...
        // 點擊頁籤
        $('.project_category__tab').on('click', function(){
            var tab = $(this);
...
$(function() {
    admin_project_categories.datatables.initialize();
});
