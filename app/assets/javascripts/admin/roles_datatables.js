...
(function() {
    ...
    this.datatables.initialize = function() {
        ...
        // 列表頁的 datatables 設定
        $('.roles_table').each(function(){
            var table = $(this);
            var tab_key = table.data('tab_key');

            var editor = new $.fn.dataTable.Editor({
                ajax: {
                    url: '/admin/roles/update_row',
                    type: 'PATCH'
                },
                table: "#"+ tab_key +"_roles_table",
                idSrc: "DT_RowId",
                fields: [
                    {
                    fieldInfo: '',
                    label: 'name',
                    label: I18n.t('activerecord.attributes.role.name'),
                    name: 'name',
                },
                {
                    fieldInfo: '',
                    label: 'resource_type',
                    label: I18n.t('activerecord.attributes.role.resource_type'),
                    name: 'resource_type',
                },
                {
                    fieldInfo: '',
                    label: 'resource_id',
                    label: I18n.t('activerecord.attributes.role.resource_id'),
                    name: 'resource_id',
                },
                ],
            });
            ...
            var datatable = table.DataTable({
                ...
                dom: 'Brtip"bottom"l',
                ajax: {
                    url: '/admin/roles/datatables.json',
                    dataSrc: 'data',
                    type: 'POST',
                    data: {
                    },
                ...
                {
                    data: 'resource_type',
                    name: 'resource_type_cont',
                    visible: true,
                    orderable: false,
                    className: 'editable col-resource_type',
                    editField: 'resource_type',
                },
                {
                    data: 'resource_id',
                    name: 'resource_id_eq',
                    visible: true,
                    orderable: false,
                    className: 'editable col-resource_id',
                    editField: 'resource_id',
                },
                    {
                        // 操作
                        data: null,
                        ...
                        render: function ( data, type, full, meta ) {
                            var id = data.id;
                            var actions = '<a href="/admin/roles/' + id + '?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-eye fa-lg"></span></a>';
                            actions += '<a href="/admin/roles/' + id + '/edit?master_show_tab='+ master_show_tab +'" class="btn btn-default m-r-10"><span class="fa fa-pencil fa-lg"></span></a>';
                            actions += '<a href="/admin/roles/' + id + '" data-confirm="確定刪除?" class="btn btn-default" rel="nofollow" data-method="delete"><span class="fa fa-trash fa-lg"></span></a>';
                            return actions;
                        },
                    },
                ],
            ...
            $("#"+ tab_key +"_roles_table").on( 'click', 'tbody td.editable', function (e) {
...
$(function() {
    admin_roles.datatables.initialize();
});
