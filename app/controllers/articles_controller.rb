...
class ArticlesController < ApplicationController
  ...
  before_action :class_authorize, only: [:new, :create]
  ...
  #  authorize Article, :index?
    ...
    respond_to do |format|
      ...
      class_scope = ArticlePolicy::Scope.new(User, Article).resolve
  ...
  def render_tab_content
    ...
    render partial: "articles/tabs/#{master_show_tab}_tab", locals: { article: @article, master_show_tab: master_show_tab }
  end

  def back
    _navigation = session[:navigation].pop
    redirect_to articles_path if _navigation.nil?
  ...
  def article_row_params(column_values)
    column_values.permit(:title, :content, :status, :published_at)
  end
    ...
    articles.map do |article|
      {
        ...
        published_at: article.published_at&.strftime('%F %T'),
      }
    end
  end
