...
class Admin::RequirementsController < ApplicationController
  ...
  before_action :set_requirement, only: [:show, :edit, :update, :destroy, :change_show_tab, :render_tab_content]
  ...
  def datatables
    authorize [:admin, Requirement], :index?
    ...
    respond_to do |format|
      ...
      @filters = DatatablesService.new({class_name: "Requirement"}).handle_filters(params)

      @keyword = params[:search][:value] unless params[:search].blank?
      @filters["id_cont".to_sym] = @keyword if @keyword

      class_scope = Admin::RequirementPolicy::Scope.new(current_user, Requirement).resolve
      ...
      @requirements = @q.result.page(@page).per(params[:length])
      ...
      format.json {
        render json: {
          ...
          data: to_datatables(@requirements) # 整理查詢結果
        }
      }
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if @requirement.save
        ...
        Attachment.update_attachable_ids(params[:attachments], @requirement.id)

        format.html {
          flash[:notice] = I18n.t('helpers.form.create_success', model: Requirement.model_name.human)
          back
        }
  ...
  def update
    respond_to do |format|
      if @requirement.update(requirement_params)
        format.html {
          flash[:notice] = I18n.t('helpers.form.update_success', model: Requirement.model_name.human)
          back
        }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html {
        flash[:notice] = I18n.t('helpers.form.destroy_success', model: Requirement.model_name.human)
        back
      }
  ...
  def update_row
    params[:data].each do |id, column_values|
      respond_to do |format|
        format.json {
          requirement = Requirement.find_by(id: id)

          authorize [:admin, requirement]

          requirement_params = requirement_row_params(column_values)

          if requirement&.update(requirement_params)
            requirements = Requirement.where(id: id)
            render json: {data: to_datatables(requirements)}
          end
        }
      end
    end
  ...
  def render_tab_content
    ...
    render partial: "admin/requirements/tabs/#{master_show_tab}_tab", locals: { requirement: @requirement, master_show_tab: master_show_tab }
  end

  def back
    _navigation = session[:navigation].pop
    redirect_to admin_requirements_path if _navigation.nil?
  ...
  def requirement_params
    params.require(:requirement).permit(:name, :datatype, :desc, :minimum, :maximum)
  end

  def requirement_row_params(column_values)
    column_values.permit(:name, :datatype, :desc, :minimum, :maximum)
  end

  # 處理呈現在datatable的資料結構
  def to_datatables(requirements)
    requirements.map do |requirement|
      {
        DT_RowId: "#{requirement.id}",
        id: requirement.id,
        name: requirement.name,
        datatype: requirement.datatype,
        datatype_i18n: requirement.datatype_i18n,
        desc: requirement.desc,
        minimum: requirement.minimum,
        maximum: requirement.maximum,
        created_at: requirement.created_at&.strftime('%F %T'),
        updated_at: requirement.updated_at&.strftime('%F %T'),
      }
    end
  end
