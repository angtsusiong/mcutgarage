...
class Admin::UsersController < AdminController
  ...
  before_action :set_user, only: [:show, :edit, :update, :destroy, :change_show_tab, :render_tab_content]
  ...
  def datatables
    ...
    respond_to do |format|
      ...
      class_scope = Admin::UserPolicy::Scope.new(current_user, User).resolve
      active_record_query = class_scope.joins(:roles).where(roles: {name: 'member'})
  ...
  def render_tab_content
    ...
    render partial: "admin/users/tabs/#{master_show_tab}_tab", locals: { user: @user, master_show_tab: master_show_tab }
  end

  def back
    _navigation = session[:navigation].pop
    redirect_to admin_users_path if _navigation.nil?
  ...
  def user_params
    params.require(:user).permit(:email, :remember_created_at, :sign_in_count, :current_sign_in_at, :last_sign_in_at, :current_sign_in_ip, :last_sign_in_ip)
  end

  def user_row_params(column_values)
    column_values.permit(:email, :remember_created_at, :sign_in_count, :current_sign_in_at, :last_sign_in_at, :current_sign_in_ip, :last_sign_in_ip)
  end
