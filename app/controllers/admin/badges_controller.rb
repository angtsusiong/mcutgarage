...
class Admin::BadgesController < ApplicationController
  ...
  before_action :set_badge, only: [:show, :edit, :update, :destroy, :change_show_tab, :render_tab_content]
  ...
  def datatables
    ...
    respond_to do |format|
      ...
      @filters = DatatablesService.new({class_name: "Badge"}).handle_filters(params)
      @keyword = params[:search][:value] unless params[:search].blank?
      @filters["id_cont".to_sym] = @keyword if @keyword
      class_scope = Admin::BadgePolicy::Scope.new(current_user, Badge).resolve
      ...
      @badges = @q.result.page(@page).per(params[:length])
      ...
      format.json {
        render json: {
          ...
          data: to_datatables(@badges) # 整理查詢結果
        }
      }
    end
  end
  ...
  def create
    ...
    respond_to do |format|
      if @badge.save
        ...
        Attachment.update_attachable_ids(params[:attachments], @badge.id)

        format.html {
          flash[:notice] = I18n.t('helpers.form.create_success', model: Badge.model_name.human)
          back
        }
        format.js {}
      else
  ...
  def update
    respond_to do |format|
      if @badge.update(badge_params)
        format.html {
          flash[:notice] = I18n.t('helpers.form.update_success', model: Badge.model_name.human)
          back
        }
        format.js {}
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html {
        flash[:notice] = I18n.t('helpers.form.destroy_success', model: Badge.model_name.human)
        back
      }
      format.js {}
    end
    ...
    params[:data].each do |id, column_values|
      respond_to do |format|
        format.json {
          ...
          authorize [:admin, badge]

          badge_params = badge_row_params(column_values)

          if badge&.update(badge_params)
            badges = Badge.where(id: id)
            render json: {data: to_datatables(badges)}
          end
        }
      end
    end
  ...
  def render_tab_content
    ...
    render partial: "admin/badges/tabs/#{master_show_tab}_tab", locals: { badge: @badge, master_show_tab: master_show_tab }
  end

  def back
    _navigation = session[:navigation].pop
    redirect_to admin_badges_path if _navigation.nil?
  ...
  def badge_row_params(column_values)
    column_values.permit(:name)
  end

  # 處理呈現在datatable的資料結構
  def to_datatables(badges)
    badges.map do |badge|
      {
        DT_RowId: "#{badge.id}",
        id: badge.id,
        name: badge.name,
        created_at: badge.created_at&.strftime('%F %T'),
        updated_at: badge.updated_at&.strftime('%F %T'),
      }
    end
  end
